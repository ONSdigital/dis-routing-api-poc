...
.PHONY: build
build: ## Builds binary of application code and stores in bin directory as dis-routing-api-poc
	go build -tags 'production' $(LDFLAGS) -o $(BINPATH)/dis-routing-api-poc
...
debug: ## Used to run code locally in debug mode
	go build -tags 'debug' $(LDFLAGS) -o $(BINPATH)/dis-routing-api-poc
	HUMAN_LOG=1 DEBUG=1 $(BINPATH)/dis-routing-api-poc
